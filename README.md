...
- 企业化 Web 前端：登录页、侧边导航、顶部用户区、指标卡、分页表格、查询工具栏、维护表单、审批操作区、智能分析区。
- 权限控制：系统管理员拥有完整管理权限；普通用户仅能查看本人住宿信息并提交/查看本人调换申请。
- 学生住宿管理：支持添加、修改、删除、按学号查询、按宿舍查询、按系/班级查询、按系和班级排序。
- 宿舍调换流程：学生提交调换申请和理由；管理员同意或拒绝；同意后自动更新学生宿舍号、宿舍电话和床位。
- MySQL 持久化：首次启动自动创建数据库、数据表和演示数据。
- 智能分析：管理员按楼栋或宿舍统计入住率、空余床位、各系比例，查询每栋楼/每个宿舍入住明细，并生成运营评估与建议。
