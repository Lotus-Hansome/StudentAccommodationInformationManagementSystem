...
- 账号体系：账号数据存储在 MySQL `users` 表，密码使用 PBKDF2 哈希保存，支持创建、修改、删除、启用/禁用账号和重置密码；学生直接使用唯一绑定的学号登录，管理员使用独立用户名且不能绑定学号；系统禁止删除当前账号，并至少保留一个启用的管理员账号。
- 权限控制：管理员拥有完整管理权限；普通用户只能查看本人住宿和同宿舍成员，并可提交/查看/撤回本人调换申请与报修反馈。
- 学生住宿管理：支持添加、修改、删除、按学号查询、按宿舍查询、楼栋+系+班级+关键词组合查询、按系和班级排序；编辑住宿信息时不需要手工填写宿舍电话，系统会按宿舍基础数据自动带出。
- 统一分页：住宿信息、调换申请、报修、床位明细、楼栋、宿舍、账号、操作日志和模型配置均支持分页；主要业务表由后端按 `page/pageSize` 返回数据，避免一次展示或加载过多记录。
...
- 数据一致性：缩减容量或停用宿舍前校验入住学生和待审批床位；学生账号只能绑定现有学号；账号状态或权限变化后旧会话立即失效。
- 操作审计：新增、修改、删除住宿信息，审批/撤回调换申请，报修处理、账号管理、楼栋管理和宿舍管理都会写入 `operation_logs`。
...
| 学生 | 学号，如 `20230001` | `student123` | 查看本人住宿与室友，提交/查看/撤回本人调换申请和报修反馈 |
